...
will load RStan, set it up to take full advantage of your computer's processing
power, and load a few packages and helper functions that we'll use later.

```{r results = 'hide'}
pkgs <- c("car", "dplyr", "ggplot2")
...
theme_set(theme_classic()) #Set default theme for ggplot2
...
There are two parts to this potential paradox. The first is that the Democrats
are the "party of the poor". What do we mean when we say that? Well, we expect
that people with lower income are more likely to vote Democrat and people with
higher income are more likely to vote Republican. Is this the case?

We'll use data from the National Annenberg 2004 Election Survey to explore
whether income is positively correlated with voting Republican. In this data
set, income is not a continuous variable, but divided into five categories: \$0
- \$20,000, \$20,000 - \$40,000, \$40,000 - \$75,000, \$75,000 - \$150,000, and
\$150,000+. Looking at the data, we see:
...
below show the proportion of all respondents in each income category (i.e. the 
...
the Stan code and data for the model are provided, so all you have to do is run
the model:
...
Republican, which lends supports to the idea that the Democrats are the party of
the poor.

However, the other part of the paradox is that the Democrats are the party of 
the rich states. In other words, can we say that states with higher average 
income more likely to vote Democrat on average? We can again use a logistic 
regression model, however instead of using the 2004 Annenberg Survey data we'll 
be using the Census CPS data. The predictor for this model will be the state 
average income level, and we can construct it so that there are the same number 
of states in each income category. The categories are: \$0 - \$39,000, \$39,000 
...
We can create the same plot that we made for the first model to look at the 
...
We see that as income level increases, the coefficients decrease. Based on our 
understanding of the inverse logit function, this means the probability of 
voting Republican decreases as well. 
...
To get more insight we'll need to add to our models. As a first step, 
for each respondent we can include their individual income and the average income 
in their state as predictors. Our outcome is still a binary indicator for voting
...
level, that is, it is the coefficient for an individual in the lowest income
category from the state with the lowest average income.
...
words, does the effect of individual income on voting Republican vary by state?

For this analysis our outcome will still be the same so we'll still
