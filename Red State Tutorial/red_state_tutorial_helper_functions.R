...
plot_coefficients_with_credible_intervals <- function(plot_title, x_labels, coefficient_result_matrix, lower_bound_percent = "2.5%", upper_bound_percent = "97.5%")
{
  ...
  plot(coefficient_result_matrix[, "mean"],
       xlim = c(1, num_coefficients + 0.25),
  ...
  ##Add bars
  arrows(1:num_coefficients, coefficient_result_matrix[, lower_bound_percent], 1:num_coefficients, coefficient_result_matrix[, upper_bound_percent], length=0.05, angle=90, code=3)
}

#Add a state's coefficients to a plot
add_state_to_plot <- function(state, state_labels, color, coefficient_result_matrix, lower_bound_percent = "2.5%", upper_bound_percent = "97.5%")
{
  points(coefficient_result_matrix[state_labels, "mean"], pch = 6)
  arrows(1:5, coefficient_result_matrix[state_labels, lower_bound_percent], 
         1:5, state_and_inc_regression_data[state_labels, upper_bound_percent], 
         length=0.05, angle=90, code=3, col = color)
  text(5, coefficient_result_matrix[state_labels[5], "mean"], labels = c(state), pos = 4)
